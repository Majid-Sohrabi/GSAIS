...
Info.Npop=250;    % Population size
Info.PCrossover=0.7;   % Crossover probability
Info.PMutation=0.1;
Info.alpha_eps=1.7;
Info.MinPts=10;
...
for j = 1:Repeat
    ...
    [Ans(j,1).MinCost, Ans(j,1).BestSol, Ans(j,1).BestCost, Ans(j, 1).pop_GA, Ans(j, 1).CPU]=Algorithm_GSAIS_NN_GQAP(Solution, Info);
    Ans(j,1).Gap_GA=(Heuristic2.Cost-Ans(j,1).MinCost)/Heuristic2.Cost;
end

%% mean
save(['Saved_Data_Quadratic_GSAIS_NN_' model_name]);